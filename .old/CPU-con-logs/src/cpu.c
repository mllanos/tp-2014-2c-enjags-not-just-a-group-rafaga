...
int main(int argc, char **argv) {
	...
	while(true) {
		...
		if (Hilo.kernel_mode)
			log_info(Logger, "Comienza a ejecutar el hilo { PID: %d, TID: %d } en modo kernel", Hilo.pid, Hilo.tid);
		else
			log_info(Logger, "Comienza a ejecutar el hilo { PID: %d, TID: %d }", Hilo.pid, Hilo.tid);
